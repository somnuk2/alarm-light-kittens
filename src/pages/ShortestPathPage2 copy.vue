          ...
          <div class="col-12">
            <q-card class="my-card q-mb-lg fit">
              ...
              <q-card-section class="bg-blue-1">
                ...
                <!-- Result (No v-html, render real components) -->
                <div v-if="result.visible" class="q-mt-md q-pa-md bg-white rounded-borders shadow-1">
                  <div class="text-subtitle1 text-center">
                    เส้นทางที่สั้นที่สุดจาก <strong>{{ result.start }}</strong>
                    ไป <strong>{{ result.end }}</strong>
                  </div>
              ...
              <q-card-section>
                ...
                <div class="row q-col-gutter-md q-mt-sm">
                  <div class="col-12 col-md-6">
                    <div class="row q-col-gutter-md">
                      <div class="col-6">
                        <q-input v-model.number="form.distance" type="number" label="ระยะทาง (เมตร)"
                          placeholder="คำนวณอัตโนมัติ" outlined dense hint="Auto-Calculated (จาก markers)">
                          <template #prepend><q-icon name="straighten" /></template>
                        </q-input>
                      </div>
                      <div class="col-6">
                        <q-input v-model.number="form.time" type="number" label="เวลา (นาที)"
                          placeholder="คำนวณอัตโนมัติ" outlined dense
...
// ---------- Derived Data (Single Source of Truth = markerList) ----------
const buildings = computed(() => {
  const b = {};
  markerList.value.forEach(loc => {
    b[loc.name] = { lat: Number(loc.lat), lng: Number(loc.lng) };
  });
  return b;
});
...
// Helpers for table display (always from markers DB)
...
const edgePolylineMap = new Map(); // key = sorted "A|B"
...
function initMap() {
  ...
  map.value = markRaw(L.map("map-path").setView([18.758, 99.015], 17));
...
function updateMapVisuals() {
  if (!map.value) return;
  if (!markersLayer.value || !edgesLayer.value) return;

  markersLayer.value.clearLayers();
  edgesLayer.value.clearLayers();
  edgePolylineMap.clear();

  // 1) Draw Markers from markers DB
  ...
  markerList.value.forEach(loc => {
    ...
    const marker = L.marker([lat, lng], { icon: greenIcon })
      .bindPopup(`<b>${loc.name}</b><br>Lat: ${lat}<br>Lng: ${lng}`);
  ...
  // 2) Draw Edges using coords from markers DB (not from edge payload)
  edgeList.value.forEach(e => {
    const A = buildings.value[e.nameA];
    const B = buildings.value[e.nameB];
    if (!A || !B) return; // if marker missing, skip
    ...
    const polyline = L.polyline(latlngs, { color: '#3388ff', weight: 3, opacity: 0.7 });
    ...
    edgePolylineMap.set(edgeKey(e.nameA, e.nameB), polyline);
  });

  if (hasNodes) map.value.fitBounds(bounds, { padding: [50, 50] });
}

function highlightPolyline(nameA, nameB) {
  const line = edgePolylineMap.get(edgeKey(nameA, nameB));
  if (!line) return;
  line.setStyle({ color: '#ef4444', weight: 6, opacity: 1 });
  line.bringToFront();
}

// ---------- Form Sync (Select A/B => coords + auto distance/time) ----------
function applySelectionToCoords() {
  ...
  if (A && B && form.selectedA !== form.selectedB) {
    ...
    const timeMin = dist / 80; // 80m/min

    form.distance = Math.round(dist);
    form.time = Math.max(1, Math.round(timeMin));
  }
}
...
function normalizeEdgePayload() {
  // Always derive coords from markers DB (single source)
  const A = buildings.value[form.selectedA];
  const B = buildings.value[form.selectedB];
  ...
  return {
    ...
    // send coords too (compatible with older backend)
    latA: A?.lat,
    lngA: A?.lng,
    latB: B?.lat,
    lngB: B?.lng,
...
function validateEdge() {
  ...
  if (form.selectedA === form.selectedB) {
    $q.notify({ type: 'warning', message: 'อาคาร A และ B ต้องไม่ใช่จุดเดียวกัน' });
    return false;
  }
  ...
  if (!form.time || isNaN(Number(form.time)) || Number(form.time) <= 0) {
  ...
  if (dup) {
    $q.notify({ type: 'warning', message: 'มีเส้นทางคู่นี้อยู่แล้วในฐานข้อมูล' });
    return false;
  }
...
async function saveData() {
  ...
  try {
    if (isEditing.value) {
      // update: ใช้ API เดิมของคุณ (ถ้า backend แยก PUT ก็เปลี่ยนได้)
      const resp = await api.post('/api/edges', payload);
      const idx = edgeList.value.findIndex(e => e.id === editingId.value);
      if (idx !== -1) edgeList.value[idx] = resp.data;
...
function editRow(row) {
  ...
  // coords always from markers DB
  applySelectionToCoords();

  // keep weights from row
...
// ---------- Load Data (DB First, fallback to constants) ----------
function fallbackEdgesFromConstants() {
  // ให้สร้างข้อมูล edge โดยอ้างอิงพิกัดจาก markers ที่มี
  // และสร้าง id ชั่วคราว (กรณีไม่มี)
...
async function loadFromBackend() {
  ...
  try {
    ...
    // cleanup: ถ้า edges บางเส้นชี้ node ที่ไม่มีใน markers => ตัดออก + แจ้งเตือน
    ...
    if (removed > 0) {
      $q.notify({ type: 'warning', message: `ตัดเส้นทางที่อ้างอิงสถานที่ที่ไม่อยู่ใน Markers ออกแล้ว ${removed} รายการ` });
    }
...
function runCalculation() {
  ...
  for (let i = 0; i < path.length - 1; i++) {
    highlightPolyline(path[i], path[i + 1]);
  }

  // totals from edgeList (distance/time)
  let totalDist = 0;
  ...
  for (let i = 0; i < path.length - 1; i++) {
    const u = path[i];
    const v = path[i + 1];
    const edge = edgeList.value.find(e =>
      (e.nameA === u && e.nameB === v) || (e.nameA === v && e.nameB === u)
    );
    if (edge) {
