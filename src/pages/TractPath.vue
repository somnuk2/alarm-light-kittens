...
function updateNavigation() {
  ...
  if (currentLocation.value) {
    ...
    // Fallback: If no location, maybe just pick a default or wait?
    // Let's just return for now, user needs to start tracking or have a loc.
    // OR we could show route from a default start if we had one.
    // Let's assume we wait for location for "Remaining" stats.
    // But we can calculate "Total" stats if we had a fixed start point.
    // The requirement says "Show Total Distance/Time when destination is set".
    // Usually this implies from "Current Position".
    return;
  }
