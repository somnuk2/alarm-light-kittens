            ...
            <q-card class="my-card q-mb-lg fit">
              ...
              <q-card-section class="relative-position">
                ...
                <q-inner-loading :showing="isLoadingMarkers">
                  ...
                  <div class="text-subtitle2 q-mt-sm">กำลังโหลดข้อมูลแผนที่...</div>
                </q-inner-loading>
              </q-card-section>
              ...
              <template #body="props">
                <q-tr :props="props" @click="editRow(props.row)" class="cursor-pointer">
                  ...
                  <q-td key="actions" :props="props" class="text-center">
                    <q-btn flat round color="primary" icon="edit" size="sm" @click.stop="editRow(props.row)" />
...
/* ---------- Cache ---------- */
const CACHE_KEY = 'graph_cache_shortest_v2';
function loadCache() {
  try {
...
const buildings = computed(() => {
  ...
  markerList.value.forEach(m => {
    ...
    b[m.name] = { lat, lng, isSafeZone: !!m.isSafeZone };
  });
  return b;
});
...
const edgePolylineMap = new Map(); // key: A|B sorted -> polyline
...
let pinnedMarker = null;              // Leaflet marker instance
let pinnedEdgeKey = null;             // string key
function clearPins() {
  // close pinned marker popup
  if (pinnedMarker) {
    try { pinnedMarker.closePopup(); } catch { }
  }
  pinnedMarker = null;

  // close pinned edge tooltip
  if (pinnedEdgeKey && edgePolylineMap.has(pinnedEdgeKey)) {
    try { edgePolylineMap.get(pinnedEdgeKey).closeTooltip(); } catch { }
  }
  pinnedEdgeKey = null;
}
...
function initMap() {
  ...
  // ✅ click on map clears pin (เหมือน UX ปกติ)
  map.value.on('click', () => {
    clearPins();
  });
...
function updateMapVisuals() {
  ...
  clearPins(); // reset pin when re-render
  ...
  // ---- Markers (hover like click, click pins) ----
  ...
  edgeList.value.forEach(e => {
    ...
    poly.on('mouseout', () => {
      if (pinnedEdgeKey !== key) poly.closeTooltip();
    });

    const popupHtml = document.createElement('div');
    popupHtml.innerHTML = `
      <div class="text-subtitle2 text-primary">เส้นทาง: ${e.nameA} - ${e.nameB}</div>
      <div class="text-caption">ระยะทาง: ${e.distance} ม. | เวลา: ${e.time} น.</div>
      <div class="row justify-end q-mt-sm">
        <button class="q-btn q-btn-item non-selectable no-outline q-btn--flat q-btn--rectangle text-negative q-focusable q-hoverable q-btn--dense"
                style="font-size: 11px;" id="btn-del-${e.id}">
          <span class="q-btn__content">ลบเส้นทางนี้</span>
        </button>
      </div>
    `;
    ...
    poly.on('click', (ev) => {
      ...
      if (pinnedEdgeKey && pinnedEdgeKey !== key && edgePolylineMap.has(pinnedEdgeKey)) {
        try { edgePolylineMap.get(pinnedEdgeKey).closeTooltip(); } catch { }
      }
      pinnedEdgeKey = key;
      poly.openTooltip();
    ...
    poly.on('popupopen', () => {
      const btn = document.getElementById(`btn-del-${e.id}`);
      if (btn) {
        btn.onclick = () => {
          poly.closePopup();
          confirmDelete(e);
...
async function saveData() {
  ...
  try {
    ...
    if (idx >= 0) {
      edgeList.value[idx] = saved;
    } else {
      // Might be undirected duplicate handled by backend update
      // Best to re-fetch if it's a new entry that might have merged
      await loadFromBackendFast();
    }

    $q.notify?.({
      type: 'positive',
      message: isEditing.value ? 'แก้ไขข้อมูลเรียบร้อย' : 'บันทึกข้อมูลสำเร็จ'
    });
    ...
    // Refresh to stay in sync anyway
    await loadFromBackendFast();
  }
}
...
async function confirmDelete(item) {
  ...
  $q.dialog({
    ...
    try {
      ...
      edgeList.value = edgeList.value.filter(e => e.id !== item.id);
